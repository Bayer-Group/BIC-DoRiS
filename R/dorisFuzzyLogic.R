...
gt <- function(x, d) {
  ...
  ###
  if (d > 0) {
    ifelse(x <= 0, 0, ifelse(x < d, lip(x, 0, 0, d, 1), 1))
  } else {
    NA
  }
}
...
lt <- function(x, d) {
  ...
  ###
  if (d < 0) {
    ifelse(x <= d, 1, ifelse(x < 0, lip(x, d, 1, 0, 0), 0))
  } else {
    NA
  }
}
  ...
  if (dn < 0 & dp > 0) {
    ifelse(
      ...
      ifelse(
        x < 0,
        lip(x, dn, 0, 0, 1),
        ifelse(x == 0, 1, ifelse(x < dp, lip(x, 0, 1, dp, 0), 0))
      )
    )
  } else {
    NA
      ...
      mapply(
        function(x, p, d) {
          ifelse(
            p == "<",
            lt(x, -d),
            ifelse(p == "=", eq(x, -d, d), ifelse(p == ">", gt(x, d), NA))
          )
        },
        x,
        p,
