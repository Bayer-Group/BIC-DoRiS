...
calc_truth_values <- function(
  ...
  if (compare_pattern == "subgroup complement") {
    ...
    list_of_mean_per_dose2 <- lapply(list_of_mean_per_dose2, function(x) {
      tmp <- Reduce(rbind, x)
    })
  ...
  alltvmatr <- apply(
    ...
    1,
    function(x) sapply(x, gettvpdose, delta),
    simplify = FALSE
  )

  ## idea: get tv for given pattern
  truthValues <- rep(NA, length(alltvmatr))
  truthValues_per_dose <- data.frame(matrix(
    NA,
    dim(Subgroup_means)[1],
  ...
  return(
    list(
      "tV results" = data.frame(
        "Subgroups" = names(alltvmatr),
        "truth Values" = truthValues
      ),
      "mean results" = Subgroup_means,
      "tv dataframe" = truthValues_per_dose
    )
  )
}
